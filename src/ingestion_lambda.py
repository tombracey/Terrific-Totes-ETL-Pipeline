...
logger.info(json.dumps({"key": Decimal("1.21")}, cls=DecimalEncoder))
...
def get_data(db, last_update):
    ...
    for table in tables:
        ...
        query = db.run(f"SELECT * FROM {identifier(table)} WHERE last_updated >= :last_update;", last_update=last_update)
