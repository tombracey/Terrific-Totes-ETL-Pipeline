...
def test_gets_data_returns_updated_rows_from_each_table(conn_fixture):
    ...
    date_3 = (current_time + datetime.timedelta(weeks= 104)).strftime('%Y-%m-%d %H:%M:%S.%f')
...
def test_get_data_ouputs_the_correct_data_intact():
    ...
    tables = ['counterparty', 'currency', 'department', 'design', 'staff', 'sales_order', 'address', 'payment', 'purchase_order', 'payment_type', 'transaction']
    for table in tables:
        assert mocked_output[table] == (dummy_counterparty_table, ["fake_column_1", "fake_column_2"])
